...
export async function handleAdmin(ctx: Ctx, req: Request, path: string, url: URL): Promise<Route | null> {
  ...
  if (seg[1] === "tasks" && seg.length >= 3) {
    ...
    if (seg[3] === "retry" && method === "POST") { const r = await one(sql`select baton.task_retry(${actor}, ${id}::uuid, ${b.reset_attempts !== false}, ${b.budget_usd == null ? null : Number(b.budget_usd)}, ${b.reason ? String(b.reason) : null}) as r`); try { await drainWebhooks(); } catch { /* ignore */ } return { status: 200, body: r }; }
