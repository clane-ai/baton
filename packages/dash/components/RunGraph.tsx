...
function Icon({ kind, x, y }: { kind: "user" | "check" | "pause" | "gear"; x: number; y: number }) {
  if (kind === "user") return <g transform={`translate(${x},${y})`} fill="none" stroke="#495057" strokeWidth={1.4}><circle cx={6} cy={4} r={3} /><path d="M0.5,13 C0.5,9 11.5,9 11.5,13" /></g>;
  if (kind === "check") return <path transform={`translate(${x},${y})`} d="M1,6 L4.5,9.5 L11,2" fill="none" stroke="#087f5b" strokeWidth={2} />;
  if (kind === "pause") return <g transform={`translate(${x},${y})`} fill="#c92a2a"><rect x={2} y={1} width={3} height={10} /><rect x={7} y={1} width={3} height={10} /></g>;
  return <g transform={`translate(${x},${y})`} fill="none" stroke="#495057" strokeWidth={1.4}><circle cx={6} cy={6} r={3} /><path d="M6,0 V2 M6,10 V12 M0,6 H2 M10,6 H12 M1.8,1.8 L3.2,3.2 M8.8,8.8 L10.2,10.2 M1.8,10.2 L3.2,8.8 M8.8,3.2 L10.2,1.8" /></g>;
}
  ...
  return (
    <div className="flow-scroll">
      <svg className="flow bpmn" width={model.width} height={model.height} viewBox={`0 0 ${model.width} ${model.height}`}>
        <defs>
          <marker id="bpmn-arrow" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="9" markerHeight="9" orient="auto"><path d="M0,0 L10,5 L0,10 z" fill="#495057" /></marker>
          <marker id="bpmn-arrow-light" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="9" markerHeight="9" orient="auto"><path d="M0,0 L10,5 L0,10 z" fill="#adb5bd" /></marker>
        </defs>
        <rect x={PAD_L} y={PAD_T - 30} width={model.width - PAD_L * 2} height={model.height - PAD_T + 20} fill="none" stroke="#b9b9b0" />
        ...
        {model.lanes.map((lane, i) => (
          <g key={lane}>
            <rect x={PAD_L} y={PAD_T + i * LANE_H} width={model.width - PAD_L * 2} height={LANE_H} fill={i % 2 ? "#faf9f5" : "#ffffff"} stroke="#dcdcd5" />
            <rect x={PAD_L} y={PAD_T + i * LANE_H} width={LANE_LABEL_W} height={LANE_H} fill="#f0f0ec" stroke="#dcdcd5" />
        ...
        {model.placed.filter((p) => p.col === model.lastCol).map((p) => (
          <path key={`e-${p.s.id}`} d={flow(p.x + NODE_W, p.y + NODE_H / 2, model.end.x - EVENT_R, model.end.y)} fill="none" stroke={p.s.state === "done" ? "#495057" : "#adb5bd"} strokeWidth={1.6} strokeDasharray={p.s.state === "done" ? undefined : "6 4"} markerEnd={p.s.state === "done" ? "url(#bpmn-arrow)" : "url(#bpmn-arrow-light)"} />
        ))}

        {/* start event */}
        <circle cx={model.start.x} cy={model.start.y} r={EVENT_R} fill="#ffffff" stroke="#495057" strokeWidth={1.6} />
        ...
        {/* end event */}
        <circle cx={model.end.x} cy={model.end.y} r={EVENT_R} fill={doneEnd ? "#e6fcf5" : "#ffffff"} stroke="#495057" strokeWidth={3.2} />
